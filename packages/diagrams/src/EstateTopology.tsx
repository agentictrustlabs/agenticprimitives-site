...
export function EstateTopology() {
  ...
  return (
    <Frame id={id} w={W} h={H} title="An estate: Home, agent runtime, vaults, edge, discovery, skills registry and chain — with relying applications around it">
      ...
      {/* Home */}
      <Box x={300} y={40} w={250} h={130} title="Home  ·  www.faithnet.me" lines={['Vercel · Next.js', 'passkey sign-in · OIDC issuer', 'ceremonies: charter, invite, delegate', 'approve parked runs · revoke', 'Today · Work · Library · Build']} tone="navy" lineSize={10.5} />
      {/* Runtime */}
      <Box x={300} y={210} w={250} h={120} title="Agent runtime  ·  a2a.faithnet.io" lines={['Cloudflare Workers + Durable Objects', 'one A2aTaskDO per agent · harness', 'playbooks by digest · triggers', 'budget-routed models (Groq / Haiku)']} tone="paper" lineSize={10.5} />
      {/* Vault */}
      <Box x={300} y={370} w={250} h={100} title="Vault  ·  MCP (private)" lines={['per-agent encrypted records', 'per-record delegation scope', 'inbox · roster · receipts · memory']} tone="paper" lineSize={10.5} />
      {/* Edge */}
      <Box x={300} y={510} w={250} h={90} title="Edge  ·  edge.faithnet.io" lines={['admission (always)', 'standard A2A surface /api/a2a/<name>', 'HTTPS req. · mTLS optional']} tone="rose" lineSize={10.5} />

      {/* Discovery + skills */}
      <Box x={640} y={40} w={250} h={110} title="Discovery  ·  discovery.faithnet.io" lines={['indexer → GraphDB', 'public, on-chain-derivable facts ONLY', '/.well-known/ard.json · ACP registry', 'A2A + MCP question surfaces']} tone="paper" lineSize={10.5} />
      <Box x={640} y={190} w={250} h={90} title="Skills registry  ·  skills.faithnet.io" lines={['archetypes → SKILL.md contracts', 'compiled AgentHarnessDefinition', 'pinned by digest; assigned at Home']} tone="paper" lineSize={10.5} />
      <Box x={640} y={320} w={250} h={90} title="KMS  ·  GCP / AWS" lines={['service keys are DELEGATES', 'session wires the custodian mints', 'revoke one wire, not an identity']} tone="paper" lineSize={10.5} />
      <Box x={640} y={450} w={250} h={150} title="faithchain  ·  rpc.faithnet.io" lines={['Besu QBFT · chain 34348 · 2 s blocks', 'AgentAccount factory · names', 'DelegationManager + enforcers', 'registries · attestations', 'Sheqel (the card room\'s coin)', '— any EVM works']} tone="teal" lineSize={10.5} />
      ...
      <Arrow id={id} d="M 550 260 C 600 260, 600 235, 638 235" label="playbook by digest" lx={594} ly={228} labelSize={9.5} />
      <Arrow id={id} d="M 550 300 C 600 300, 600 365, 638 365" label="signs as delegate" lx={594} ly={348} labelSize={9.5} />
