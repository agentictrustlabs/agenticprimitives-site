...
export const DEMOS: readonly Demo[] = [
  ...
  },
  {
    ...
    script: [
      ...
      { do: 'Ask: “Find an agent that can help with a study on justification.”', expect: 'Your agent searches the estate’s discovery tier and returns candidate agents — the Publications catalog among them — with what each one can do and the evidence for it. Nothing has been asked of them yet.' },
      { do: 'Ask the Publications catalog the same question through your agent.', expect: 'The catalog answers with titles, teachers and topics. Your agent reports what it read and from which tier; the run leaves a receipt in your vault.' },
  ...
  {
    id: 'home-mcp2',
    name: 'Registry → Publications in Claude',
    kind: 'Home MCP · discovery + A2A content',
    line: 'Search the ADR registry through your agent, then ask Publications (or another publisher) over A2A — inside Claude.',
    blurb:
      'The same Home MCP connector as “Your agent, inside Claude,” exercised on the content path: discover_agents searches the estate’s ADR / discovery registry; engage sends your words as you to the returned agent’s A2A endpoint. Works for Publications catalog asks and other publisher agents the registry lists. Claude never holds your keys — it holds a revocable ask-as-me delegate; the hop and the answer are your agent’s record.',
    links: [
      { href: HOME_MCP_CONNECTOR, label: 'Connector address', note: 'Claude → Settings → Connectors → Add custom connector' },
      { href: 'https://claude.ai/chat/8377ba9f-f9c0-44a3-bb64-1def58db4e2a', label: 'Example chat', note: 'grace / works query via Publications' },
      { href: 'https://faithnet.me', label: 'Sign the ask-as-me grant at your Home' },
    ],
    proves: [
      'Discovery is a registry search through your agent — candidates with evidence, not a vendor score',
      'Engage is A2A: Publications (or another publisher) answers from its own catalog under its own playbook',
      'Claude shows the tool hop (“Searched Publications resources”); the receipt lands in your vault',
    ],
    sampleQuery:
    ...
    shot: {
      src: '/shots/home-mcp-publications-claude.jpg',
      alt: 'Claude using the Home MCP connector — searched Publications resources, then answered a grace-and-works question from the publisher’s catalog',
      caption: 'claude.ai — connector on; registry hop → Publications A2A; answer cites the publisher, not the model’s memory alone',
      width: 1024,
      height: 539,
    },
    script: [
      ...
      { do: 'Paste the sample query (grace alone / good works).', expect: 'Claude calls the connector — you see a hop like “Searched Publications resources.” The answer is grounded in the Publications catalog, with sources named as theirs.' },
      { do: 'Or step it: “Find agents that offer study plans about justification.” Then engage the Publications agent with the same ask.', expect: 'Registry candidates first (ADR discovery); then Publications’ own A2A reply — titles, teachers, topics — presented as that agent’s answer.' },
