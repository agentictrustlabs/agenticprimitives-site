...
- [Thesis](${SITE.url}/thesis): build an ecosystem where every participant expands what everyone can accomplish — the competing bet is to coordinate whole journeys through independent participants while each retains control of its contributions; healthcare, fitness, faith, finance and travel; the ecosystem builder as customer; platform vs network-of-integrations vs shared domain foundation; the strategy (shared foundation, domain layer, ecosystem experience, delivery sequence); thirteen bets each with its test; eleven design principles; the layered ontology; initial proofs and measures per domain; where the bet can fail
- [A critical review](${SITE.url}/thesis/review): the project's own critical reading of the thesis on its merits — a new offering, conceded; the three testable claims inside the vision; the platform as the competitor and estates as the difference; the five markets on their premises; the thirteen bets on their reasoning; the eleven principles as principles (consistent, thin on governance); what the thesis does not say; what would prove it; what a doubter is right about
...
- [Versus · who could assemble it](${SITE.url}/compare/assemble): competitive analysis of the horizontal agentic market, dated September 2026 with a projection to early 2027 — the capability map — nine areas, some forty functions, and the package that covers each (identity, authority, harness and orchestration, memory and context, triggers, edge and protocols, registry, evidence and credentials, coordination, messaging and collaboration, library and content, Home, ontology, vault and private storage, public knowledge, KMS, developer kit); the field dated (Okta Agent SSO / XAA GA Aug 2026, Entra Agent ID and Agent 365, AWS AgentCore Policy and Consent Portal, Cloudflare OS / Wallets / cloudflare.pay / Identity-Aware AI Gateway, Google AP2, Anthropic Managed Agents, OpenAI Agents SDK, Visa TAP and Mastercard Agent Pay, ERC-8004 and x402, Keycard / Scalekit, AGNTCY / NANDA / MCP registry, LangSmith Deployment); four assemblies that could produce something similar (Cloudflare-native, enterprise identity, hyperscaler platform, Web3) with what each would have and lack; a function-by-function scorecard across all areas; sixteen deficiencies dated to the window; what no assembly produces
