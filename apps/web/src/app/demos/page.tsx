...
export default function Demos() {
  return (
    <>
          ...
          <nav className="grid gap-2 text-sm" aria-label="Demos">
            {DEMOS.map((d, i) => (
              <a key={d.id} href={`#${d.id}`} className="card-dark flex items-center justify-between !p-4 hover:border-white/25">
                <span>
                  <span className="num-mark mr-3 text-brass">0{i + 1}</span>
      ...
      {DEMOS.map((d, i) => (
        <Section key={d.id} id={d.id} tone={i % 2 === 0 ? 'white' : 'ink'} number={`0${i + 1}`} eyebrow={d.kind} title={d.name} lede={d.line} wide>
