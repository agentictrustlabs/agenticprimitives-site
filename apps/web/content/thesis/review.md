...
| 8 | One vocabulary; behaviour generated, authority never; versions bound | **Holds inside one estate.** Across organizations it is a bet; version binding in the mandate is designed, not shipped. |
...
The claim: intent, capability, relationship, receipt and trust edge are IRIs in one ontology; code binds by IRI and a build gate fails on an invented term; the interface, the planner's tools and the agent card are generated from one skill definition.

The evidence for this is the estate's own incident history: every recent failure was a *knowledge* failure — a treasury found by name similarity when the relationship existed, a receipt written to a field nothing read — and not an authority failure. One definition removes the class of bug where the assistant and the product disagree.

Two things a doubter should press. Four generated surfaces can agree perfectly about the wrong behaviour, and a build gate catches an invented term, not a valid term used wrongly; consistency concentrates error, and only independent behavioural validation — the live gates with their negative twins — catches it. And a mandate that binds an intent digest but not the ontology version or the adapter that interprets it can be reinterpreted after approval by changing either; version binding in the mandate is designed, not shipped, and until it ships the compiler and the release process are security-critical components the thesis does not name as such.

Across organizations the honest position is that shared meaning for *data* may not be worth its cost when models can translate between local schemas cheaply. Shared meaning for *authority* is different: a mandate over "a payment to a member" must mean the same thing to the payer's verifier and the payee's, and translation at act time is where an injection lives. The thesis makes that argument; a critical reading notes that it is an argument, not yet a demonstration.
