...
The sharpest problem is inside the substrate's own beachhead, and it is a problem of *which chain*, not of chains. `memberOf`, `stewardOf` and `charteredUnder` are recorded on chain by `agent-relationships`, typed as hierarchical, transitive and symmetric. On a public chain that would be a machine-readable map of who belongs to which congregation and who stewards which mission organization — for a population the Field application protects with a "discreet posture." Today the graph is on the private estate chain, which is the right place for it; what the substrate lacks is the *public projection* of that private graph: a rule that relationships are also vault credentials presented under grant, and that what crosses to the public chain is a commitment — a set root, an existence proof, a zero-knowledge membership proof — never the edge. That is a design addition rather than a reversal, and it should ship before any public-chain anchoring of the estate.
...
**Neutral ground against discreet posture — resolved by dimension, not by choosing.** As first written, this review assumed the relationship graph had to live on the same ledger as the root, so that a neutral root implied a public map of membership. That assumption was wrong, and the substrate's own code shows why. Custody, authority, admission and resolution each have a private dimension and a public projection. A principal's account is *counterfactual* — its address is computable from its custody configuration, a passkey or a social recovery set, and the substrate verifies its signatures before deployment (ERC-6492) — so identity, delegation and relationship can all be defined, signed and verified off chain against that address without a chain footprint. The chain enters when an effect must be *enforced* (value at commit) or *projected* (a public claim strangers should see), and those are two different chains: a private estate chain for enforcement without exposure, and a public chain for the public projection — anchored receipts, admissions, reviews, provenance commitments. Membership never has to be a public fact for a grant to be verifiable.

What survives of the objection is narrower and still real. First, a private enforcement chain is neutral only to the extent its validators are the domain's own institutions rather than one operator; the layering does not remove the governance question, it locates it. Second, a verifier *outside* the estate — the hotel that has never seen the traveller's Home — cannot read the private chain, so cross-estate verification needs either a public commitment (a revocation root, a membership-set root, anchored on the public chain) or a zero-knowledge proof of non-revocation or membership against such a root. That is exactly the shape Privacy Pools' association sets have — prove you belong to a set without revealing which member you are — and it is the right tool for the public projection of a private graph. Third, the shielded-value stack (Privacy Pools, Railgun, Aztec's private smart contracts, and the Kohaku wallet framework that integrates them) addresses *payment* privacy; only Aztec-style private state addresses *graph* privacy on a public chain, and its maturity should be verified at the time of adoption rather than assumed. The estate's private chain is the near-term answer; the public privacy stack is the path to doing the same on neutral public ground.
...
**Change, in the substrate.** Split rotation from recovery in the custody epoch. Give relationships a vault form beside the private-chain record, and a public projection that is a commitment or a membership proof, never the edge. Bind schema and execution-semantics versions into the mandate. Add a per-resource aggregate enforcer. Name the conformance check for open intents and the logical request identity for nonces. Decide what "neutral" means for the private enforcement chain — a consortium the domain's institutions validate — and anchor its public projections on neutral public ground.
